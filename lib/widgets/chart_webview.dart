...
class _ChartWebViewState extends State<ChartWebView> {
  ...
  /// Smart X-axis tick selection (Issue #70)
  /// Returns list of indices that should show labels
  List<int> _getOptimalTickIndices(int dataLength, {int maxTicks = 10}) {
    // Issue #70: Smart X-axis labeling requirements
    if (dataLength <= 5) {
      // ≤5 data points: Show actual data point labels
      return List.generate(dataLength, (index) => index);
    }

    // >5 data points: Calculate 5 equidistant dates between first and last entries
    final ticks = <int>[];
